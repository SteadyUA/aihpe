...
8.  **Asset Handling and Path Flattening**:
    ...
    - Specifically, check image files (using \`list_files\` mime-type output) to ensure they have the correct file extension (e.g., '.png', '.jpg'). If an image lacks an extension or has an incorrect one, instruct the execution agent or plan a \`move_files\` job to rename it with the correct extension before referencing it. CRITICAL: You MUST also explicitly instruct the execution agent to search for the original extensionless filenames inside JS arrays or string concatenations and update them to include the new extension.
