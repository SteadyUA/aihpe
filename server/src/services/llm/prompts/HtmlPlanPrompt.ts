...
2. **Initial Plan**: Use 'edit_memory_file' to create a flat, high-level action 'plan.md'. This file MUST track all steps (completed, in-progress, pending).
...
5. **Review**: The subagent will return a result (success or failure). Update 'plan.md' using 'edit_memory_file' to mark the step as done or failed.
...
6. **Merging JavaScript and HTML Scripts (CRITICAL)**: NEVER ask a subagent to extract multiple modules, merge logic, or make massive edits in a single step. Instead:
    ...
    - **Targeted Edits**: If you need to remove tracking scripts or update 'fetch' calls, dispatch subagents to use targeted edits ('edit_file') based on line numbers found during discovery, rather than extracting and rewriting large chunks of code.
...
9. **State Management**: If you need to pass data between steps, you can write it into a specific section of 'plan.md' or use the 'edit_memory_file' tool. 
10. **Failure Handling**: If 'run_subagent' returns a failure, do NOT just retry blindly. Read the error and the subagent's failure report to understand what it was trying to do and why the task was too complex. Update the 'plan.md' to break the task down further, and issue a simpler or more targeted instruction.
