import { streamText, tool, ModelMessage, LanguageModel, ImagePart, TextPart } from 'ai';
import { z } from 'zod';
    ...
    async generatePage(request: GeneratePageRequest): Promise<GeneratePageResult> {
        ...
        const tools: Record<string, any> = {
            read_file: tool({
                ...
                inputSchema: z.object({
                    file: z.enum(['index.html', 'styles.css', 'script.js']).describe('The file to read'),
                    summary: z.string().describe('Explain why you need to read this file. This will be shown to the user.'),
                }),
                execute: async ({ file, summary }: { file: 'index.html' | 'styles.css' | 'script.js', summary: string }) => {
            ...
            edit_file: tool({
                description: 'Edit a file by replacing exact string match. The oldString must match exactly one location in the file.',
                inputSchema: z.object({
                    file: z.enum(['index.html', 'styles.css', 'script.js']).describe('The file to edit'),
                    oldString: z.string().describe('The exact string to replace. Must be unique in the file.'),
                    newString: z.string().describe('The new string to replace it with.'),
                    summary: z.string().describe('Explain why you are making this edit. This will be shown to the user.'),
                }),
                execute: async ({ file, oldString, newString, summary }: { file: 'index.html' | 'styles.css' | 'script.js', oldString: string, newString: string, summary: string }) => {
                    let content = '';
                    ...
                    if (content.split(targetString).length > 2) return `Error: oldString found multiple times in ${file}. Provide more unique context.`;
                    
                    const newContent = content.replace(targetString, newString);
                    if (file === 'index.html') currentFiles.html = newContent;
                    else if (file === 'styles.css') currentFiles.css = newContent;
                    else if (file === 'script.js') currentFiles.js = newContent;
                    return `Successfully updated ${file}`;
                },
            }),
            summary: tool({
                description: 'Call this when you are done making changes to provide a summary of what you did to the user.',
                inputSchema: z.object({
                    message: z.string().describe('The summary message to display to the user.'),
                }),
        ...
        if (request.allowVariants) {
            tools.generate_variants = tool({
                ...
                execute: async (args: { count: number, instructions: string[] }) => {
                    variantRequest = args;
        ...
        try {
            ...
            while (steps < maxSteps) {
                ...
                const cachedTokens = usage.cachedInputTokens ?? usage.promptTokensDetails?.cachedTokens;
                ...
                console.log(`Context Usage:     ${usedTokens.toLocaleString()} / ${limit.toLocaleString()} tokens`);
                ...
                for (const m of stepMessages) {
                    ...
                    if (m.role === 'assistant' && Array.isArray(m.content) && request.onProgress) {
                ...
                if (toolCalls && toolCalls.length > 0) {
                     ...
                     const executedToolCallIds = new Set(
                        stepMessages
                            ...
                            .flatMap(m => Array.isArray(m.content) ? m.content.map((c: any) => c.toolCallId) : [])
                     );

                     // Filter out tool calls that were already executed
                     const pendingToolCalls = toolCalls.filter(tc => !executedToolCallIds.has(tc.toolCallId));
                     ...
                     if (pendingToolCalls.length > 0) {
                         ...
                         if (request.onProgress) {
                             ...
                             pendingToolCalls.forEach(tc => {
                                 ...
                                 try {
                                     ...
                                     if (args && typeof args === 'object' && args.summary) {
                         ...
                         const toolResults = await Promise.all(pendingToolCalls.map(async tc => {
                             ...
                             if (tool && tool.execute) {
                                 try {
                                     ...
                                     if (!input) {
                                         throw new Error(`No input provided for tool ${tc.toolName}`);
                                     }
                         ...
                         const summaryCalled = pendingToolCalls.some(tc => tc.toolName === 'summary');
                         const variantsCalled = pendingToolCalls.some(tc => tc.toolName === 'generate_variants');
                         ...
                         const summaryExecuted = toolCalls.some(tc => tc.toolName === 'summary');
                         if (summaryExecuted) {
                             break;
                         }
                     }
