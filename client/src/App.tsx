...
export default class App extends React.Component<AppProps, AppState> {
  ...
  componentDidUpdate(_prevProps: AppProps, prevState: AppState) {
    ...
    if (prevState.groups !== this.state.groups) {
      localStorage.setItem('sessionGroups', JSON.stringify(this.state.groups));
    }
  }
  ...
  handleSessionChange = () => {
    ...
    if (!activeSessionId) {
      this.setState({ messages: [], files: { html: '', css: '', js: '' }, utilStatus: 'idle', utilMessage: null, selection: null, isPicking: false });
      return;
    }
    ...
    this.evtSource.addEventListener('chat-status', (e) => {
      ...
      if (data.status === 'started' || data.status === 'generating') {
        this.setState({ utilStatus: 'busy', utilMessage: data.message || null });
        ...
        this.setState({ utilStatus: 'error', utilMessage: data.message || 'Error occurred' });
      }
    });
  ...
  createSession = async () => {
    try {
      ...
      this.setState(prevState => ({
        ...
        groups: session.group ? { ...prevState.groups, [session.id]: session.group } : prevState.groups
      }));
    } catch (error) {
  ...
  cloneSession = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/sessions/${activeSessionId}/clone`, { method: 'POST' });
  ...
  cloneVersion = async (version: number) => {
    ...
    try {
      const res = await fetch(`/api/sessions/${activeSessionId}/versions/${version}/clone`, { method: 'POST' });
  ...
  previewVersion = async (version: number) => {
    ...
    try {
      this.setState(prevState => ({
        ...
        utilStatus: 'busy',
        activeVersions: { ...prevState.activeVersions, [activeSessionId]: version }
      }));

      const res = await fetch(`/api/sessions/${activeSessionId}/versions/${version}/files`);
  ...
  removeSession = (id: string) => {
    this.setState(prevState => ({
      ...
      activeSessionId: prevState.activeSessionId === id ? null : prevState.activeSessionId
    }));
  };
  ...
  sendMessage = async (text: string) => {
    ...
    this.setState(prevState => ({
      utilStatus: 'busy',
      messages: [...prevState.messages, { role: 'user', content: text, selection: selectionData }],
      selection: null, // Clear selection after sending
      activeVersion: null, // Reset to follow latest
      activeVersions: { ...prevState.activeVersions, [activeSessionId]: null }
    }));
    ...
    try {
      await fetch('/api/chat', {
        ...
        body: JSON.stringify({ sessionId: activeSessionId, message: text, selection: selectionData })
      });
    } catch (e) {
      this.setState({ utilStatus: 'error' });
    }
  ...
  render() {
    const { sessions, activeSessionId, messages, files, utilStatus, selection, isPicking } = this.state;
    ...
    return (
      <div className="app">
        <div style={{ gridColumn: '1 / -1', borderBottom: '1px solid var(--border-color)', background: 'var(--bg-color)' }}>
          <SessionBar
            sessions={sessions}
            ...
            statusMap={Object.fromEntries(
              this.state.sessions.map(id => [id, id === this.state.activeSessionId ? this.state.utilStatus : 'idle'])
            )}
